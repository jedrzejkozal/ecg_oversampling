...
def load():
    ...
    C3 = np.argwhere(trainY == 3).flatten()

    #class 3 oversampling
    additional_samples = np.apply_along_axis(augment, axis=1, arr=trainX[C3]).reshape(-1, 187)
    additional_labels = np.ones(shape=(additional_samples.shape[0],), dtype=int)*3
    trainX = np.vstack([trainX, additional_samples])
    trainY = np.hstack([trainY, additional_labels])

    trainX, trainY, _, _ = choose_n_samples_from_each_class(trainX, trainY, 800)
...
def choose_n_samples_from_each_class(dataX, dataY, n):
    ...
    dataX_result = np.vstack([dataX[subC0], dataX[subC1], dataX[subC2], dataX[subC3], dataX[subC4]])
    dataY_result = np.hstack([dataY[subC0], dataY[subC1], dataY[subC2], dataY[subC3], dataY[subC4]])

    dataX_remaining = np.vstack([dataX[remainingC0], dataX[remainingC1], dataX[remainingC2], dataX[remainingC3], dataX[remainingC4]])
    dataY_remaining = np.hstack([dataY[remainingC0], dataY[remainingC1], dataY[remainingC2], dataY[remainingC3], dataY[remainingC4]])
